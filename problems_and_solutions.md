...
- **引入 Stealth 插件**：集成 `playwright-stealth` 库，并修复了其版本更新后的 `Stealth().apply_stealth_async(page)` 调用方式，抹除自动化指纹。
...
- **动态监测**：脚本在导航到页面后，会检测是否存在验证码或加载中状态，给予用户最长 30 秒的窗口进行手动点击校验，验证通过后程序会自动继续执行后续逻辑。
...
- **事件模拟**：该方法会完整触发 React 或富文本框架（如 ProseMirror）所需的 input、focus 等事件，确保内容被前端逻辑正确接收。
...
- **按需导航**：只有在 Agent 第一次接收任务时才会执行 `goto`（开启新对话），后续所有迭代消息直接在同一个 Page 页面寻找输入框并发送。这实现了在同一页面内进行多轮优化，极大提高了连贯性和运行效率。
...
- **手动清理指令**：提供 `pkill -f "Google Chrome"` 等指令建议，确保 Profile 目录在每次脚本启动前处于未占用状态。
...
### 解决方案
- **自动保存 Markdown**：逻辑中增加了 `save_results` 函数，将每一轮的 Prompt、评价和最终生成的回答，按照时间戳自动保存至 `./output` 目录下的 `.md` 文件。
- **持续对话模式**：将 `main.py` 的执行逻辑改为 `while True` 循环。程序完成后不会关闭浏览器，而是等待用户输入下一个问题，从而允许在同一个浏览器 Session 中连续工作。
